...
namespace Information_System.Controllers
{
    public class CreateController : Controller
    {
        ...
        public string url = "http://10.145.163.14/";
        ...
        public CreateController()
        {
            try
            {
                url = System.Web.HttpContext.Current.Request.Url.GetLeftPart(UriPartial.Authority).Equals(null) ? "" : System.Web.HttpContext.Current.Request.Url.GetLeftPart(UriPartial.Authority);
            }
            catch
            {
                url = "";
                        ...
                        using (SqlCommand cmd = new SqlCommand(null, conn))
                        {
                            ...
                            try
                            {
                                ...
                                if (action == "Edit" || action == "Revise")
                                {
                                    ...
                                    cmd.Parameters.AddWithValue("@WARNING_DATE", (model.WARNING_DATE == null) ? (object)DBNull.Value : (object)model.WARNING_DATE.Value.AddHours(8));
                                }
                                else
                                {
                                    ...
                                    cmd.Parameters.AddWithValue("@WARNING_DATE", (model.WARNING_DATE.Value.AddHours(8) == null) ? (object)DBNull.Value : (object)model.WARNING_DATE.Value.AddHours(8));
                                }
                                conn.Open();
                                cmd.ExecuteNonQuery();
                ...
                using (SqlCommand cmd = new SqlCommand(null, conn))
                {
                    ...
                    try
                    {
                        ...
                        if (reader.Read())
                        {
                            ...
                            model.WARNING_DATE = Convert.ToDateTime(Convert.ToDateTime(reader["WARNING_DATE"]).AddHours(8).ToString("dd/MMM/yyyy hh:mm"));
                        }
                        reader.Close();
            ...
            else
            {
                ...
                using (SqlConnection con = new SqlConnection(Fn.conRTCStr))
                {
                    ...
                    using (SqlCommand cmd = new SqlCommand(null, con))
                    {
                        ...
                        query += " ISSUE_ID = " + Fn.getSQL(Session["emp_id"].ToString()) + " order by ISSUE_DATE desc";
                        ...
                        while (dr.Read())
                        {
                            if (dr.HasRows)
                            {
                                ...
                                {
                                    ViewData["warn"] = Convert.ToDateTime(dr["WARNING_DATE"]).AddHours(8).ToString("dd/MMM/yyyy HH:mm:ss");
                                }
                        ...
                        if (id != null)
                        {
                            cmd.CommandText = $@"SELECT * FROM [db_rtc].[dbo].[TBL_TECH_IS_REQUEST] a1
                                              LEFT JOIN [db_rtc].[dbo].TBL_TECH_IS_DOCINFO a2
                                             ON a1.IS_ID = a2.REQUEST_NO  where a1.IS_ID = '{id}' OR a2.ID = '{id}'";
                            dr = cmd.ExecuteReader();
                            if (dr.HasRows)
                            {
                                while (dr.Read())
                            ...
                            if (dr.Read())
                            {
                                ...
                                fp.txt_ATT_DOC_IMPORTANT = dr["ATT_DOC_IMPORTANT"].ToString();
                                dr.Close();
                                int countse = 0;
                                if(Newid != null)
                                {
                            ...
                            //cmd.CommandText = " SELECT TOP(1) * FROM TBL_TECH_IS_REQUEST WHERE  ISSUE_ID = " + Fn.getSQL(Session["emp_id"].ToString()) + " ORDER BY ISSUE_DATE DESC";
                            //dr = cmd.ExecuteReader();
                            //while (dr.Read())
                            //{
                            //    dm.REASON_EXPLAIN = dr["REASON_EXPLAIN"].ToString();
                            ...
                            while (dr.Read())
                            {
                                ...
                                if (dr["WARNING_DATE"].ToString() != "")
                                {
                                    ViewData["warn"] = Convert.ToDateTime(dr["WARNING_DATE"]).AddHours(8).ToString("dd/MMM/yyyy hh:mm");
                                }
                    ...
                    else if (action == "Edit")
                    {
                        using (SqlConnection con = new SqlConnection(Fn.conRTCStr))
                        {
                            ...
                            using (SqlCommand cmd = new SqlCommand(null, con))
                            {
                                ...
                                if (doc.ATT_DOC_IMPORTANT != null)
                                {
                                    sql.AppendLine(" , ATT_DOC_IMPORTANT = @ATT_DOC_IMPORTANT ");
                                }
                                sql.AppendLine(" , STATUS = 'CREATE' ");
                                sql.AppendLine(" WHERE ID = " + Fn.getSQL(Newid));
                                cmd.CommandText = sql.ToString();
                                cmd.Parameters.AddWithValue("@DOC_DETAIL", (doc.DOC_DETAIL == null) ? (object)DBNull.Value : (object)doc.DOC_DETAIL);
                                cmd.Parameters.AddWithValue("@EFFECTIVE", (doc.EFFECTIVE == null) ? (object)DBNull.Value : (object)doc.EFFECTIVE);
                                cmd.Parameters.AddWithValue("@ADD_ORDER_NO", (doc.ADD_ORDER_NO == null) ? (object)DBNull.Value : (object)doc.ADD_ORDER_NO);
                                ...
                                //}
                                sql.Clear();
                                sql.AppendLine(" UPDATE TBL_TECH_IS_DOCINFO_APPROVE SET STATUS = NULL WHERE DOCINFO_ID = " + Fn.getSQL(Newid));
                                cmd.CommandText = sql.ToString();
                                cmd.ExecuteNonQuery();
                                sql.Clear();
                                //foreach (var itemfiles in doc.ATT_DOC_OTHER)
                                ...
                                {
                                    sql.Clear();
                                    sql.AppendLine(" DELETE FROM TBL_TECH_IS_DOCINFO_APPROVE WHERE DOCINFO_ID =" + Fn.getSQL(Newid) + " AND APPROVE_LEVEL IS NULL ");
                                    cmd.CommandText = sql.ToString();
                                    cmd.ExecuteNonQuery();
                                    ...
                                    foreach (var list in approveList)
                                    {
                                        approveQueryList += "INSERT INTO TBL_TECH_IS_DOCINFO_APPROVE(DOCINFO_ID, APPROVE_ID) VALUES(" + Fn.getSQL(Newid) + ", " + Fn.getSQL(list) + " ) ";
                                    }
                                ...
                                string[] ATT_DOC_OTHER = new string[5];
                                sql.Clear();
                                sql.AppendLine(" SELECT PIC_REF_1, PIC_REF_2, ATT_DOC_PURCHASE, ATT_DOC_REQUIRE , ATT_DOC_IMPORTANT FROM TBL_TECH_IS_DOCINFO WHERE ID = " + Fn.getSQL(Newid));
                                cmd.CommandText = sql.ToString();
                                SqlDataReader dr = cmd.ExecuteReader();
                                if (dr.Read())
                                {
                                    doc.txt_PIC_REF_1 = dr["PIC_REF_1"].ToString();
                                    doc.txt_PIC_REF_2 = dr["PIC_REF_2"].ToString();
                                    doc.txt_ATT_DOC_PURCHASE = dr["ATT_DOC_PURCHASE"].ToString();
                                    doc.txt_ATT_DOC_REQUIRE = dr["ATT_DOC_REQUIRE"].ToString();
                                    doc.txt_DOC_IMPORTANT = dr["ATT_DOC_IMPORTANT"].ToString();
                                }
                    ...
                    else if (action == "Revise")
                    {
                        using (SqlConnection con = new SqlConnection(Fn.conRTCStr))
                        {
                            ...
                            using (SqlCommand cmd = new SqlCommand(null, con))
                            {
                                ...
                                //ยกเลิกเอกสารใบเก่า
                                sql.Clear();
                                sql.AppendLine(" UPDATE TBL_TECH_IS_DOCINFO SET STATUS = 'REVISE' WHERE ID =" + Fn.getSQL(Newid) + " AND INFO_NO = " + Fn.getSQL(doc.INFO_NO));
                                cmd.CommandText = sql.ToString();
                                cmd.ExecuteNonQuery();
                                ...
                                rev_template += "<a href='" + url + "/TE-IS/Approve/InformationFormat/" + Newid + "' >Click the link to view details " + doc.INFO_NO + "</a>";
                                ...
                                dr.Close();
                                string PIC_REF_1 = null, PIC_REF_2 = null, ATT_DOC_PURCHASE = null, ATT_DOC_REQUIRE = null, ATT_DOC_IMPORTANT = null;
                                string[] ATT_DOC_OTHER = new string[5];
                                sql.Clear();
                                sql.AppendLine(" SELECT PIC_REF_1, PIC_REF_2, ATT_DOC_PURCHASE, ATT_DOC_REQUIRE , ATT_DOC_IMPORTANT FROM TBL_TECH_IS_DOCINFO WHERE ID = " + Fn.getSQL(Newid));
                                cmd.CommandText = sql.ToString();
                                ...
                                if (dr.Read())
                                {
                                    ...
                                    ATT_DOC_IMPORTANT = dr["ATT_DOC_IMPORTANT"].ToString();
                                }
                                dr.Close();
                                ...
                                if (ATT_DOC_IMPORTANT != "" && ATT_DOC_IMPORTANT != null)
                                {
                                    string sourceFile = System.IO.Path.Combine(sourcePath, ATT_DOC_IMPORTANT);
                                    string destFile = System.IO.Path.Combine(sourcePath, random + "_" + ATT_DOC_IMPORTANT.Substring(7));
                                    System.IO.File.Copy(sourceFile, destFile, true);
                                    ATT_DOC_IMPORTANT = random + "_" + ATT_DOC_IMPORTANT.Substring(7);
                                }
                                int count = 0;
                                sql.Clear();
                                sql.AppendLine(" SELECT PATCH FROM TBL_TECH_IS_FILES WHERE IS_ID   = " + Fn.getSQL(Newid));
                                cmd.CommandText = sql.ToString();
                                dr = cmd.ExecuteReader();
                                if (dr.Read())
                                {

                                    ATT_DOC_OTHER[count] = dr["PATCH"].ToString();
                                    count++;
                                }

                                dr.Close();
                                foreach (var itemfiles in ATT_DOC_OTHER)
                                {
                                    if (itemfiles != "" && itemfiles != null)
                                    {
                                        string sourceFile = System.IO.Path.Combine(sourcePath, itemfiles);
                                        string destFile = System.IO.Path.Combine(sourcePath, random + "_" + itemfiles.Substring(7));
                                        System.IO.File.Copy(sourceFile, destFile, true);
                                        ATT_DOC_OTHER[count] = random + "_" + itemfiles.Substring(7);
                                        count++;
                                    }
                                }
            ...
            try
            {
                ...
                using (SqlConnection conn = new SqlConnection(Fn.conRTCStr))
                {
                    ...
                    using (SqlCommand cmd = new SqlCommand(null, conn))
                    {
                        ...
                        //while (reader.Read())
                        //{
                        //    fp.txt_PIC_REF_1 = reader["PIC_REF_1"].ToString();
                        //    fp.txt_PIC_REF_2 = reader["PIC_REF_2"].ToString();
                        //    fp.txt_ATT_DOC_PURCHASE = reader["ATT_DOC_PURCHASE"].ToString();
                        //    fp.txt_ATT_DOC_REQUIRE = reader["ATT_DOC_REQUIRE"].ToString();
                        //    fp.txt_ATT_DOC_IMPORTANT = reader["ATT_DOC_IMPORTANT"].ToString();

                        //}
                        //reader.Close();
                        //cmd.CommandText = " UPDATE TBL_TECH_IS_DOCINFO SET ATT_DOC_IMPORTANT = '' , ATT_DOC_REQUIRE = '' , ATT_DOC_PURCHASE = ''  WHERE ID = " + Fn.getSQL(id) +"";
                        //cmd.ExecuteNonQuery();
            ...
            try
            {
                using (SqlConnection conn = new SqlConnection(Fn.conRTCStr))
                {
                    ...
                    using (SqlCommand cmd = new SqlCommand(null, conn))
                    {
                        ...
                        reader.Close();
                        cmd.CommandText = " UPDATE TBL_TECH_IS_DOCINFO SET PIC_REF_1 = ''  WHERE ID = " + Fn.getSQL(id) + " ";
                        cmd.ExecuteNonQuery();
        ...
        public JsonResult deleteFile2( string id)
        {
            List<string> fileList = new List<string>();
            string filePart = Server.MapPath("~/Temp_File/").ToString();
                ...
                using (SqlConnection conn = new SqlConnection(Fn.conRTCStr))
                {
                    ...
                    using (SqlCommand cmd = new SqlCommand(null, conn))
                    {
                       ...
                       SqlDataReader reader = cmd.ExecuteReader();
                        ...
                        reader.Close();
                        cmd.CommandText = " UPDATE TBL_TECH_IS_DOCINFO SET PIC_REF_2 = ''  WHERE ID = " + Fn.getSQL(id) + " ";
                        cmd.ExecuteNonQuery();
        ...
        public JsonResult deleteFile3(string id)
        {
            List<string> fileList = new List<string>();
            string filePart = Server.MapPath("~/Temp_File/").ToString();
            try
            {
                using (SqlConnection conn = new SqlConnection(Fn.conRTCStr))
                {
                    ...
                    using (SqlCommand cmd = new SqlCommand(null, conn))
                    {
                        ...
                        reader.Close();
                        cmd.CommandText = " UPDATE TBL_TECH_IS_DOCINFO SET ATT_DOC_PURCHASE = ''  WHERE ID = " + Fn.getSQL(id) + " ";
                        cmd.ExecuteNonQuery();
        ...
        public JsonResult deleteFile4( string id)
        {
            List<string> fileList = new List<string>();
            string filePart = Server.MapPath("~/Temp_File/").ToString();
            ...
            try
            {
                using (SqlConnection conn = new SqlConnection(Fn.conRTCStr))
                {
                    ...
                    using (SqlCommand cmd = new SqlCommand(null, conn))
                    {
                        ...
                        reader.Close();
                        cmd.CommandText = " UPDATE TBL_TECH_IS_DOCINFO SET ATT_DOC_IMPORTANT = ''  WHERE ID = " + Fn.getSQL(id) + " ";
                        cmd.ExecuteNonQuery();
        ...
        public JsonResult deleteFile5( string id)
        {
            List<string> fileList = new List<string>();
            string filePart = Server.MapPath("~/Temp_File/").ToString();
            ...
            try
            {
                ...
                using (SqlConnection conn = new SqlConnection(Fn.conRTCStr))
                {
                    ...
                    using (SqlCommand cmd = new SqlCommand(null, conn))
                    {
                        ...
                        reader.Close();
                        cmd.CommandText = " UPDATE TBL_TECH_IS_DOCINFO SET ATT_DOC_REQUIRE = ''  WHERE ID = " + Fn.getSQL(id) + " ";
                        cmd.ExecuteNonQuery();
            ...
            using (SqlConnection con = new SqlConnection(Fn.conRTCStr))
            {
                ...
                using (SqlCommand cmd = new SqlCommand(null, con))
                {
                    try
                    {
                        ...
                        {
                            string uriDestination = string.Empty ;
                            //Upload file into SERVER//
                            try
                            {
                                string ftp = "ftp://10.145.163.14/";
                                string ftpFolder = "IS/";
                                foreach (string list in model.FILE_LIST)
                                {
                                    ...
                                    {
                                         uriDestination = ftp + ftpFolder + list;
                        ...
                        else
                        {
                            ...
                            if (model.APPROVE_STATUS == "APPROVED")
                            {
                                ...
                                activity = "<a href='" + url + "/TE-IS/Create/ReceivedRequirment/" + model.IS_ID + "'> Click link to approve requirement </a>";
                            }
        ...
        [HttpPost]
        public bool saveInformation(DocInfoModel doc)
        {
            using (SqlConnection con = new SqlConnection(Fn.conRTCStr))
            {
                con.Open();
                using (SqlCommand cmd = new SqlCommand(null, con))
                {
                    ...
                    try
                    {
                        ...
                        cmd.CommandText = $" UPDATE TBL_TECH_IS_DOCINFO SET STATUS = 'IN-PROGRESS' WHERE ID = '{doc.ID}'";
                        cmd.ExecuteNonQuery();
                        ...
                        if (dr.Read())
                        {
                            ...
                            try
                            {
                                dm.txt_PIC_REF_1 = doc.PIC_REF_1.FileName;
                            ...
                            try
                            {
                                dm.txt_PIC_REF_2 = doc.PIC_REF_2.FileName;
                            ...
                            try
                            {
                                dm.txt_ATT_DOC_PURCHASE = doc.ATT_DOC_PURCHASE.FileName;
                            ...
                            try
                            {
                                dm.txt_ATT_DOC_REQUIRE = doc.ATT_DOC_REQUIRE.FileName;
                            ...
                            try
                            {
                                dm.txt_DOC_IMPORTANT = doc.ATT_DOC_IMPORTANT.FileName;
                            }
                            catch
                            {
                        ...
                        cmd.CommandText = $@"SELECT TOP (5) [PATCH]
                                      FROM [db_rtc].[dbo].[TBL_TECH_IS_FILES] where [IS_ID] = '{doc.ID}'";
                        dr = cmd.ExecuteReader();
                        if(dr.HasRows)
                        {
                            while(dr.Read())
                        ...
                        //UpDATEไฟล์ ที่เพิ่มเข้าไป
                        cmd.CommandText = $@" UPDATE [db_rtc].[dbo].[TBL_TECH_IS_DOCINFO]
                                       SET 
                                          [PIC_REF_1] = '{dm.txt_PIC_REF_1}'
                                          ,[PIC_REF_2] = '{dm.txt_PIC_REF_2}'
                                          ,[ATT_DOC_PURCHASE] = '{dm.txt_ATT_DOC_PURCHASE}'
                                          ,[ATT_DOC_REQUIRE] = '{dm.txt_ATT_DOC_REQUIRE}'
                                          ,[ATT_DOC_IMPORTANT] = '{dm.txt_DOC_IMPORTANT}'
                                    where ID = '{doc.ID}'";
                        dr = cmd.ExecuteReader();
                        if (dr.HasRows)
                        {
